...
def istft(
    ...
    if n_frames == 1 and not center: # only one frame
        ytmp = stft_matrix / (window.view(1, 1, n_fft)+1e-20)
        ytmp = ytmp.squeeze(1)
    ...
    end = end if center else y.shape[-1]
    ...
    # )
    y = (y / (window_envelop + 1e-20)).squeeze(1)  # size (channel, expected_signal_len)
...
# @torch.jit.script
def spectrogram(
    waveform, pad, window, n_fft, hop_length, win_length, power, normalized, center
):
    # type: (Tensor, int, Tensor, int, int, int, int, bool, bool) -> Tensor
    r"""
    ...
    spec_f = _stft(
        waveform, n_fft, hop_length, win_length, window, center, "reflect", False, True
    )
    if normalized:
...
def biquad(waveform, b0, b1, b2, a0, a1, a2):
    ...
    Args:
        ...
        b0 (float): numerator coefficient of new input, x[n]
        b1 (float): numerator coefficient of input one time step ago x[n-1]
        b2 (float): numerator coefficient of input two time steps ago x[n-2]
        a0 (float): denominator coefficient of new output y[n], typically 1
        a1 (float): denominator coefficient of new output y[n-1]
        a2 (float): denominator coefficient of new output y[n-2]
