...
class Spectrogram(torch.nn.Module):
    ...
    def __init__(self, n_fft=400, win_length=None, hop_length=None,
                 ...
                 power=2, normalized=False, symmetry=True, center=True, wkwargs=None):
        ...
        window = window_fn(self.win_length + symmetry)[symmetry:] if wkwargs is None else window_fn(self.win_length, **wkwargs)
        self.window = window
        self.pad = pad
        self.power = power
...
class AmplitudeToDB(torch.jit.ScriptModule):
    ...
    def __init__(self, stype='power', top_db=None):
        ...
        self.top_db = top_db # torch.jit.Attribute(top_db, Optional[float])
...
class MelScale(torch.nn.Module):
    ...
    def forward(self, specgram):
        ...
        mel_specgram = torch.matmul(specgram.transpose(1, 2), self.fb.to(specgram.device)).transpose(1, 2)
        return mel_specgram
...
class MelSpectrogram(torch.nn.Module):
    ...
    def __init__(self, sample_rate=16000, n_fft=400, win_length=None, hop_length=None, f_min=0., f_max=None,
                 pad=0, n_mels=128, window_fn=torch.hann_window, symmetry=True, center=True, wkwargs=None):
        ...
        self.spectrogram = Spectrogram(n_fft=self.n_fft, win_length=self.win_length,
                                       ...
                                       normalized=False, symmetry=symmetry, center=center, wkwargs=wkwargs)
    ...
    def forward(self, waveform):
        ...
        Returns:
            torch.Tensor: Mel frequency spectrogram of size (channel, ``n_mels``, time)
        """
        specgram = self.spectrogram(waveform) # same as stft(waveform).mag().pow(2)
...
class MFCC(torch.nn.Module):
    ...
    def forward(self, waveform):
        ...
        mfcc = torch.matmul(mel_specgram.transpose(1, 2), self.dct_mat.to(mel_specgram.device)).transpose(1, 2)
        return mfcc
...
class TimeStretch(torch.jit.ScriptModule):
    ...
    def __init__(self, hop_length=None, n_freq=201, fixed_rate=None):
        ...
        self.phase_advance = phase_advance # torch.jit.Attribute(phase_advance, torch.Tensor)
